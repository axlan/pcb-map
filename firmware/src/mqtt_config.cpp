...
public:
    ...
    void maintain() {
        if (port_==0) return;
        bool connected = (WiFi.status() == WL_CONNECTED) && (WiFi.localIP() != INADDR_NONE);
...
void ProcessUDPCommands() {
  ...
  if (packetSize > 0) {
    ...
    ESP_LOGI(TAG, "CMD from %s:%d → %s", remoteIp.toString().c_str(), remotePort, input);
...
void MQTTConfigManager::Begin(const MQTTConfigCallback& on_change){
  ...
  ESP_LOGI(TAG, "MQTT Config service registered: mqtt-config._udp.local:%u", MQTT_UDP_CMD_PORT);
}
