...
MQTTClientManager::MQTTClientManager(const char* client_name, const std::vector<String>& topics, MQTT_CALLBACK_SIGNATURE, const MQTTConfig& config)
    : mqtt_client_secure_(wifi_client_secure_), mqtt_client_insecure_(wifi_client_), last_mqtt_attempt_ms_(0),  client_name_(client_name), config_(config), topics_(topics) {

    mqtt_client_secure_.setCallback(callback);
    mqtt_client_insecure_.setCallback(callback);

    // This opens the possibility for man in the middle attacks to compromise the encryption.
    // The alternative would be to set the Root CA certificate for the broker(s) being used.
    // Handling that dynamically would be fairly memory intensive. Ideally, if that mattered, it would make more sense to hardcode the broker. 
    wifi_client_secure_.setInsecure();

    mqtt_client_= config_.use_tls ? &mqtt_client_secure_ : &mqtt_client_insecure_;
}

void MQTTClientManager::Update() {
    // Handle MQTT connection and reconnection
    if (mqtt_client_->connected()) {
        mqtt_client_->loop();  // Process incoming messages and keep connection alive
    } else {
...
void MQTTClientManager::OnConfigChange(const MQTTConfig& config) {
    ESP_LOGI(TAG, "[MQTT] Config updated: %s:%d (TLS: %s, User: %s)",
                  ...
                  config.use_tls ? "yes" : "no", config.user_name.c_str());
